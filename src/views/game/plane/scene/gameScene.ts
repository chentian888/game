...
export default class GameScene extends Phaser.Scene {
  ...
  preload() {
    this.load.image('gameBG', new URL('../image/gameBG.png', import.meta.url).href)
    this.load.image('myBullet', new URL('../image/bomb.png', import.meta.url).href)
    this.load.image('bulletSupply', new URL('../image/bullet_supply.png', import.meta.url).href)
    this.load.image('bombSupply', new URL('../image/bomb_supply.png', import.meta.url).href)
    this.load.spritesheet('myPlane', new URL('../image/myPlane.png', import.meta.url).href, {
      frameWidth: 66,
      frameHeight: 82
    })
    this.load.spritesheet('smallPlane', new URL('../image/smallPlane.png', import.meta.url).href, {
      frameWidth: 34,
      frameHeight: 24
    })
    this.load.spritesheet('midPlane', new URL('../image/midPlane.png', import.meta.url).href, {
      frameWidth: 46,
      frameHeight: 64
    })
    this.load.spritesheet('bigPlane', new URL('../image/bigPlane.png', import.meta.url).href, {
      frameWidth: 110,
      frameHeight: 170
    })
    this.load.image('startBtn', new URL('../image/start_btn.png', import.meta.url).href)
    this.load.audio('do', new URL('../audio/ndo.mp3', import.meta.url).href)
    this.load.audio('bulletQuantity', new URL('../audio/chi.mp3', import.meta.url).href)
    this.load.audio('bigboom', new URL('../audio/bigboom.mp3', import.meta.url).href)
    this.load.audio('gameover', new URL('../audio/gameover.mp3', import.meta.url).href)
    this.load.audio('bgm', new URL('../audio/bgm.mp3', import.meta.url).href)
  }
  create() {
    this.sounds.do = this.sound.add('do')
    this.sounds.bulletQuantity = this.sound.add('bulletQuantity', { volume: 0.6 })
    ...
    this.physics.add.overlap(
      this.myBullets,
      this.enemyPlanes1,
      this.enemyAndMyBulletCollision,
      null,
      this
    )
    this.physics.add.overlap(
          ...
          ? this.myBulletQuantity % 2 != 0 && i > this.myBulletQuantity / 2 - 1
            ...
            : this.player.x -
              (this.myBulletQuantity -
                i -
                this.myBulletQuantity / 2 -
                (this.myBulletQuantity % 2 != 0 ? 0.5 : 0)) *
                20
          : this.player.x +
            (i - this.myBulletQuantity / 2 + (this.myBulletQuantity % 2 != 0 ? 0.5 : 1)) * 20
      // 从对象池取子弹
      const tmpMyBullet = this.myBullets.get(
        x,
  ...
  enemyAndMyPlaneCollision(myPlane, enemyPlane) {
    if (enemyPlane.hp > 0 && enemyPlane.active) {
      ...
      let startBtn = this.add
        .sprite(viewWidth / 2, viewHeight / 2, 'startBtn')
        .setInteractive()
        .setScale(0.5)
